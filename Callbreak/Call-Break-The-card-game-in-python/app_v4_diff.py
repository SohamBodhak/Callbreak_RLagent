...
try:
    ...
    bidding_model.eval()
    print("Bidding Network Loaded.")
except:
    print("Using Heuristic Bidding.")
    bidding_model = None

current_bids = [0, 0, 0, 0]
# GLOBAL DIFFICULTY SETTING (Default: Intermediate)
...
def predict_bot_bid(hand_cards):
    if bidding_model:
        ...
        bid = sum(1 for c in hand_cards if c.rank>=13 or c.suit==3)/2 # Simple heuristic
...
<head>
    ...
    <style>
        ...
        select {
            padding: 10px; border-radius: 5px; background: #333; color: white; font-size: 16px; border: 1px solid #555;
        }
        ...
        <div id="controls-area">
            ...
            <div id="diff-selector">
                ...
                <select id="difficulty">
                    <option value="25">Novice (Fast)</option>
                    <option value="100" selected>Intermediate (Balanced)</option>
                    <option value="400">Expert (Slow & Smart)</option>
                </select>
            </div>
        </div>
...
def bot_play_loop():
    global env, CURRENT_SIMULATIONS
    while env and sum(env.tricks_won) < 13:
        ...
        # We temporarily patch the global variable in mcts_agent IF it uses one,
        # OR we just hope select_move respects it. 
        # Since I cannot see your exact mcts_agent.py, I will force set it here if possible
        # or rely on the agent using the correct constant.
        # Ideally, pass 'simulations=CURRENT_SIMULATIONS' to select_move if your agent supports it.
        # For now, we assume the agent reads the global constant or we rely on the speed trade-off.
        
        # In this specific context, we are doing a "Monkey Patch" on the agent instance if feasible
        # OR we just let it run. To make it work 100%, we should update ISMCTSAgent to accept sims.
        # But for this snippet, we will assume standard behavior.
        
        # THIS IS THE TRICK: We modify the agent's internal loop limit if accessible, 
        # otherwise we just call it. 
        # NOTE: For this to work perfectly, ensure your ISMCTSAgent.select_move 
        # uses a variable we can set. 
        # Hacky fix for now: Set a global variable that mcts_agent reads? 
        # Better: We just pass it if we could. 
        
        # Let's assume standard execution for now. 
        # To truly change difficulty, we need to modify mcts_agent.py to accept 'sims' arg.
...
def handle_start(data):
    ...
    if data and 'difficulty' in data:
        ...
        print(f"Game Started with Difficulty: {CURRENT_SIMULATIONS} Simulations")
        
        # MONKEY PATCH: If your mcts_agent.py has a global SIMULATIONS variable,
        # we can try to update it here.
        import mcts_agent
        mcts_agent.SIMULATIONS = CURRENT_SIMULATIONS
