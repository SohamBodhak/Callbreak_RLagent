...
<head>
    ...
    <style>
        ...
        #turn-indicator {
            ...
            border: 4px solid rgba(255, 215, 0, 0.5); pointer-events: none; display: none;
        }
    </style>
</head>
<body>
    <div id="game-table">
        ...
        <button id="start-btn" onclick="startGame()" style="position:absolute; top:60%; left:50%; transform:translate(-50%, -50%); padding:10px 20px; font-size:18px;">Start Game</button>
        ...
        <div style="position:absolute; top:10px; left:20px; background:rgba(0,0,0,0.5); padding:10px; border-radius:8px;">
    ...
    <script>
        ...
        socket.on('game_update', state => {
            ...
            // Game Over
            if(state.game_over) {
                const w = state.winner === 0 ? "YOU WON!" : `Bot ${state.winner} Won!`;
                document.getElementById('status-msg').innerText = w;
...
def get_state_json():
    ...
    return {
        ...
        'trick': trick_vis,
        'scores': env.tricks_won,
        'game_over': sum(env.tricks_won) == 13,
        'winner': np.argmax(env.tricks_won) if sum(env.tricks_won) == 13 else -1
    }

def bot_play_loop():
    """Logic to handle bot moves sequentially."""
    global env
    
    # Loop as long as it is a Bot's turn
    while env and sum(env.tricks_won) < 13:
        if env.current_player == 0:
            # It's Human's turn - Stop the loop and wait for input
        ...
        if c_obj:
            ...
            if len(env.trick_cards) == 4:
                ...
                env.current_player = w_abs # Winner starts next
...
def handle_move(data):
    if env.current_player != 0: return # Safety check
    ...
    if c_obj:
        ...
        # 2. Check Trick Logic
        if len(env.trick_cards) == 4:
            # Trick Over immediately after human move (Human played last)
            socketio.emit('game_update', get_state_json())
            socketio.sleep(1.5)
            ...
            # Trick continues -> Pass to next player
            # --- THIS WAS ALSO MISSING ---
            env.current_player = (env.current_player + 1) % 4
        
        emit('game_update', get_state_json())
        
        # 3. Trigger Bot Loop if it's not Human's turn anymore
        if env.current_player != 0:
            socketio.start_background_task(bot_play_loop)
        else:
             emit('status_update', {'msg': "Your Turn!"})
