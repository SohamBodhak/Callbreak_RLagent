...
# Assuming you saved the A3C MCTS code in mcts_agent_a3c.py
from mcts_agent_a3c import ISMCTSAgentA3C, ActorCritic 
...
def run_tournament():
    print(f"--- STARTING TOURNAMENT: MCTS (A3C) vs 3 HEURISTIC BOTS ---")
    ...
    mcts_bot = ISMCTSAgentA3C()
    
    if mcts_bot.model is None:
        print("CRITICAL ERROR: A3C Model not found. Train A3C first!")
        return
    ...
    for game_idx in range(1, NUM_GAMES + 1):
        ...
        while not terminated:
            # Player 0 (MCTS-A3C) thinks and plays
        ...
        winner_name = "MCTS-A3C" if winner_idx == 0 else f"Bot {winner_idx}"
    ...
    results = [
        (0, total_tricks_won[0], "MCTS (A3C)", total_games_won[0]),
    ...
    if winner[0] == 0:
        print(f"\n>>> VICTORY! The A3C-MCTS Agent won by {diff} tricks!")
    else:
        diff_to_winner = results[0][1] - total_tricks_won[0]
        print(f"\n>>> DEFEAT. The A3C-MCTS Agent lost by {diff_to_winner} tricks.")
